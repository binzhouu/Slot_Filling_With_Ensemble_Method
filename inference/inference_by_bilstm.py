...
dset_path = os.path.join(ROOT_PATH, 'saved_models/slot_filling/bilstm_crf/data/alphabet.dset')
model_dir = os.path.join(ROOT_PATH, 'saved_models/slot_filling/bilstm_crf/bilstm_crf.model')


class SlotModelV1(object):
	def __init__(self):
		...
		self.configs = self.read_configs(model_num=1)
		...
		self.model = BiLstmCrf(data, self.configs)
		assert self.configs['encoder_type'] == 'bilstm_crf'
...
if __name__ == '__main__':
	texts = [
		'打开香薰机的灯', '洗衣机', '电视机', '客厅', '空调', '打开客厅空调', '晾衣机升高20%', '晾衣杆的高度调高2米', '跟彩云城又尤为患者', '六月十日',
		'将客厅的空调调高2度', '我要看CCTV6', '设置空调为制热模式', '红色', '10秒', '到10秒', '调高一点', 'go go go', '晾衣机升高20%', '晾衣杆调高点',
	...
	# texts = ['4周后我想要一下打扫床铺的服务', '呼叫3天后打扫的服务']
	# intents = ['room_clean', 'room_clean']
	start = datetime.now()
	slot_filling = SlotModelV1()
